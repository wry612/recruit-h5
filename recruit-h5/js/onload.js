...
var setPage=function(nr_) {
      if( SWF_ADDRESS == "true" ){ 
		  ...
		  var address = results.attr('data-address');
           ...
           if($('.page-container').css('opacity')==1&& $('#fb7-ajax').css('z-index')==4){
           	if (!window.paused) {
...
var Book_v7 = {
    ...
    ready: function(){
	  ...
	  updateProcess(65);
       ...
       WIDTH_BOOK=Number(config['page_width']);  	 
       ...
       $(window).bind('orientationchange resize', function(event){
        ...
        $("#fb7 .fb7-cont-page-book").css('width',(WIDTH_BOOK)+'px');
        ...
        $("#fb7 .fb7-page-book").css('width',(WIDTH_BOOK-(paddingL+paddingR))+'px');
		...
		$('#fb7-book').turn({
			...
			when: {
				...
				turned: function(e, page) {
					$(".audio-wrap").off("touchend").on("touchend", function(e) {
						if ($(".audio-wrap").hasClass('audio-ani')) {
							$(".audio-wrap").removeClass('audio-ani');
							var audio = document.getElementById("audio");
							window.paused=true;
							audio.pause();
						} else {
							$(".audio-wrap").addClass('audio-ani');
							var audio = document.getElementById("audio");
							window.paused=false;
							audio.play();
						}
					...
					$('.page').find('.rb-corners').removeClass('animation');
					$('.page'+page).find('.rb-corners').addClass('animation');
                    ...
                    if(page == $(this).turn('pages')-1){
                    	...
                    	$('.next-btn').on('touchend',function(){
                    		$('#fb7-ajax').css('z-index',0);  
                    		$('body').off('touchmove touchstart');                  		
                    	});
                    }
                    $('.pic7-box .pic7').hide();
                    $('.pic7-box .pic7-1').show();
                    if(page == 7){
                    	setTimeout(function(){
                    		$('.pic7-box .pic7-1').fadeOut(500,function(){
	                    		$('.pic7-box .pic7-2').fadeIn(500,function(){
	                    			$('.pic7-box .pic7-2').fadeOut(500,function(){
	                    				$('.pic7-box .pic7-3').fadeIn(500);
	                    			});
	                    		});
	                    	});
                    	},200);
                    ...
                    $('.job-box').removeClass('job-item-ani');
                    if(page<$(this).turn('pages')-1 && $('.page-container').css('opacity')=='1'){
                    	if (!window.paused) {
                    		var audio = document.getElementById("audio");
							window.paused=false;
							audio.play();
					...
					}else if(page==$(this).turn('pages')-1){                  	
						$('.job-box').addClass('job-item-ani');
                     }
                    var audio = document.getElementById("audio");
	...
	arrows: function() {
		$('.fb7-nav-arrow.prev').on('touchend',function() {
			//$('#fb7-book').turn('previous');
            Book_v7.prevPage();
            Book_v7.resize_page();
		});
		$('.fb7-nav-arrow.next').on('touchend',function() {
			//$('#fb7-book').turn('next');
            Book_v7.nextPage();
            Book_v7.resize_page();
		});
    ...
    touchend_book:function(e){   
    	$('.page'+$('#fb7-book').turn('page')).find('.rb-corners').addClass('animation'); 
    },    
    ...
    scale_arrows:function(){
       ...
       var height_arrow=this.heightBook();
	   ...
	   var newX=0;
	   arrow_next.css('right',newX+'px');
	    ///position arrow left
	   var arrow_prev=$('#fb7 .fb7-nav-arrow.prev');
	   var newX=0;
	   arrow_prev.css('left',newX+'px');
    ...
    getAutoZoomBook: function(arrow_width_){
		...
		var screen_width=  $("#fb7").width();
		var screen_height= $("#fb7").height();
...
var showCode=function(flag){
	$('.code.code'+flag).show();
	$('.qrcode-tip.qrtip'+flag).css('display','inline-block');
	$('.cover-box').show();
};
...
window.onload = function() {
	updateProcess(100);
	$('.skip-btn').on('touchend', function(event) {
		var audio1 = document.getElementById("audio1");
			audio1.play();
		var audio = document.getElementById("audio");
			window.paused=false;
	...
	$(".qrcode-div").css('height',$(window).height()/2-500);
	// $('.list-header').on('touchend', function(event) {
		// var index = $(this).parent().index()+1;
		// showCode(index);
	// });
	$('.code img').on('touchmove touchend', function(event) {
		event.preventDefault();
		return false;
	});
	$('.cover-box').on('touchmove', function(event) {
		event.preventDefault();
		return false;
	});
	$('.cover-box').on('touchend', function(event) {
		$(this).hide();
		$('.code').hide();
		$('.qrcode-tip').hide();
		event.preventDefault();
